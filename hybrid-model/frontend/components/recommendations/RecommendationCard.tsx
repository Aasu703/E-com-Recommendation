...
export function RecommendationCard({ product }: { product: RecommendedProduct }) {
  return (
    <Link href={`/product/${product.product_id}`} className={`card ${product.in_stock ? "" : "stock"}`}>
      {product.is_festival_recommendation && <Badge tone="festival">Festival Pick</Badge>}
      ...
      <p className="muted">Rating {product.avg_rating?.toFixed(1) ?? "new"} • Score {product.hybrid_score.toFixed(2)}</p>
